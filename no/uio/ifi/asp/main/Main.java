...
public class Main {
	...
	System.out.println("This is the IN2030 Asp interpreter skrevet av AlexBrul (" +
			   version + ")");
	...
	if(testParser) System.out.println("Testparser er true");
    ...
    private static void doRunInterpreter(Scanner s) {
		System.out.println("Starter intrepetering av fil alex");
	AspProgram prog = AspProgram.parse(s);
	if(true){ //if (log.doLogPrettyPrint) {
	    log.prettyWriteLn();
	    prog.prettyPrint();
	}
